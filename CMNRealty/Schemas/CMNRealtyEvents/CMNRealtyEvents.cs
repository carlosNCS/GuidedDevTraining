namespace Terrasoft.Configuration
{
    ...
    [EntityEventListener(SchemaName = "CMNRealty")]
    public class RealtyEntityEventListener : BaseEntityEventListener
    {
        public override void OnSaving(object sender, EntityBeforeEventArgs e)
        {
            ...
            decimal price = realty.GetTypedColumnValue<decimal>("CMNPrice");
            if (price > 1000000000)
            {
                e.IsCanceled = true;

                string messageTemplate = new LocalizableString(realty.UserConnection.ResourceStorage,
                    "CMNRealtyEvents", "LocalizableStrings.ValueIsTooBig.Value").ToString();
